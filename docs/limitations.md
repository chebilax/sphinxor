...
**What it does not claim**, and these are the reason this file keeps growing rather than shrinking:

- **Not that every mechanism is read.** An unreadable SpEL expression, a `RouterFunction` builder's routes, an external interface's mappings — each is *announced* and not understood. Reading their content is a separate decision per case, and several are recorded below as open.
- **Not completeness of the list itself.** A Spring Security mechanism ADR 0029 does not enumerate cannot be `silent` under it. The list is something to add a line to, not a proof.
- **Nothing about the items ADR 0029 §1 puts out of scope** — other frameworks, imperative in-handler checks, JAX-RS routing, project-local authorization annotations, Kotlin source. Those are documented below and a zero on them is correct behaviour, not a gap.
- **Nothing about NestJS**, which has no equivalent enumeration.

One open question sits outside that checklist entirely, because it changes *what is scanned* rather than *what is understood*: **should test sources be analyzed at all?** `parseProject` skips directories named `test` and files ending `Test`/`Tests`/`IT`. That is a partial, undeclared policy rather than a decision — other test layouts are scanned and counted today — and it deserves an ADR of its own. It surfaced from the nested-controller measurement, where all 33 corpus instances turned out to sit in already-skipped paths.
...
- **A controller declared as a nested class — fixed by [ADR 0034](decisions/0034-nested-controllers.md).** `extractControllers` walked top-level `class_declaration` nodes only, so a `@RestController` nested inside another class yielded no endpoints *and no controller* — invisible rather than incomplete, so [ADR 0032](decisions/0032-controllers-that-yield-no-routes.md)'s detection could not see it either. It now walks classes at any depth. *This entry previously claimed the corpus contains no instance; it contains **33**, across apollo, conductor, nacos, shenyu, spring-cloud-dataflow and halo.* All 33 sit in paths `parseProject` already skips (32 in a `test` directory **and** matching `*Test.java`, one by the directory alone), so the fix is a **zero-delta change against the corpus** and exists to protect production code.
...
- **Functional routing (`RouterFunction`) — announced since [ADR 0033](decisions/0033-functional-routing.md).** Spring WebFlux's alternative to an annotated controller: routes are built in code with `route(GET("/x"), handler::method)` rather than declared by annotation. Extraction reads none of it, and now says so. Measured with the extractor: **82 builder methods in three repositories** — halo 77, shenyu 4, JeecgBoot 1 — where the unit is a method whose declared return type is `RouterFunction`, the only criterion spanning all three idioms found (a `@Bean`, an implementation of a project interface, and the interface declaration itself).

  The warning counts **methods, not routes**, and is worded as indicative: one builder chain declares any number of routes (halo's 104 chains carry at least 200 verb predicates), and a method may be a fragment composed into a chain elsewhere rather than an independent declaration.

  **halo shows this warning alongside ADR 0019 §2's "recognized no endpoints".** That is not redundancy: the first says the matrix is empty, the second says why and would fire on a full matrix too — shenyu recognizes 394 endpoints and JeecgBoot 931, and neither triggers the emptiness notice while both lose functional routes.
