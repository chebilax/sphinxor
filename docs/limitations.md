...
**`@RequestMapping(method = …)` is now the load-bearing one.** It blocks three separate things at once: the 381 method-level declarations below, `eladmin`'s `@Anonymous*Mapping`, and `shenyu`'s `@Shenyu*Mapping` — the latter two being meta-annotations ADR 0024 deliberately does not resolve *because* they bottom out here and would surface nothing.
