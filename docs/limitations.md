...
- **A controller declared as a nested class.** `extractControllers` walks top-level `class_declaration` nodes only, so a `@RestController` nested inside another class contributes no endpoints at all. Found while tracing an unrelated three-endpoint discrepancy, and **the corpus contains no instance** — 20 repositories, zero nested controller classes — so it is recorded rather than fixed. It is a real shape in Java, and the failure is the silent one: no warning distinguishes it from a class with no routes.
