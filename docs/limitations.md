...
- **A method-level `@RequestMapping` with no `method` attribute** — **164** uses across 17 repositories. In Spring each maps *every* HTTP verb. Representing one faithfully means either eight endpoint rows per handler, of which the developer probably intended one or two, or a new "any verb" concept in the model with consequences for identity, the Cerbos exporter and `mutating-endpoint-without-access-control`. ADR 0026 §4 deliberately left this open rather than bundling a second model decision; it is measured and recorded, with both options and no verdict.
