...
- **`@RequestMapping(method = RequestMethod.X)`** — **415** route declarations in the 14-repository Spring corpus and **593** across all 20 (JeecgBoot 271, inlong 175, thingsboard 93, nakadi 45, nacos 6, metersphere 3). ADR 0011 §1 names this as a deliberate scope cut, so it is not a surprise; its *scale* is. It also means **the one repository where the model works is only partly seen**: thingsboard mixes both shapes, so `POST /api/customer` — `@PreAuthorize("hasAuthority('TENANT_ADMIN')")`, plainly readable — is absent from its model entirely.

  *An earlier version of this entry said 381, and that figure was wrong twice over.* It counted eladmin's 5 and microcks's 4 as route declarations when they were the **meta-annotation declarations themselves**, not uses in any controller; and it counted annotation occurrences rather than routes, so `method = {GET, POST}` — which declares two — counted once. The figures above are controller-scoped and verb-expanded. Of the 339 mutating routes among them, **155 are already covered** by rules that exist: 44 by Spring Security guards (all thingsboard) and 111 by [ADR 0023](decisions/0023-third-party-authorization-annotations.md)'s Shiro recognition. **184** would be new findings across 20 repositories, 74 of them in the 14-repository Spring corpus.
