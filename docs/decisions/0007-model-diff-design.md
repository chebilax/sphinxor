...
**Amended by** [`0036-became-public-gates-ci.md`](0036-became-public-gates-ci.md), which adds a third regression case: an endpoint protected in base and unprotected in head, not allowlisted in head. §3 placed "endpoints that became public" under the *informational* structural diff, and that placement is what this amendment reverses — nothing else about §3 moves, and no rule's confidence changes. The warrant is in §3's own text: it describes the failure mode it was guarding against as an implementation that would "silently fail to gate a real loss of protection", and `README.md` was claiming that gate existed for the whole of 0.7.x while a removed `@PreAuthorize` exited 0.
