...
| `diff/keys.go`, `diff/diff.go`, `diff/regressions.go` | role refs, guard keys, finding subjects | **Unchanged — deferred by name.** `guardAppKey` is (endpoint, guard name, scope) and none of those moves, so no existing diff result changes. A permission added or removed between two runs is invisible to `sphinxor diff` until step 2, and that is a stated gap, not an oversight. **Superseded by [ADR 0037](0037-permissions-in-the-diff.md)**, which closes it: `diff/keys.go` and `diff/diff.go` change, `diff/regressions.go` does not, and nothing new gates. |
| `report/diff.go` | diff result | **Unchanged**, for the same reason. **Superseded by [ADR 0037](0037-permissions-in-the-diff.md)** §2, which adds one section. |
