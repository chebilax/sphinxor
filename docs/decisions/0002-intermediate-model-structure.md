...
**Amended by** [`0035-permissions-in-the-model.md`](0035-permissions-in-the-model.md), which adds a `permissionReferences` collection and a `DeclaresPermissions` field to `guardApplications` — a named requirement that is neither a guard nor a role, which the shape above had no slot for. That ADR deliberately adds no `permissionDeclarations` counterpart: across the 20-repository Spring corpus no project declares a permission in source (the registries are database seeds), so the "no declaration found" state this ADR's Shared points insist on is, for permissions, the only state there is.
